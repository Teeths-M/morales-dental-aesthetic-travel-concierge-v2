    ...
    <div className="min-h-screen" style={{ background: '#F8FAF9' }}>
      ...
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-xl border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[9px] font-bold uppercase tracking-widest text-emerald-700">IQ200 SYSTEM</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
          {statCards.map(({ label, value, color, bg, IconComp, isText }) => (
            <div key={label} className={`rounded-2xl ${bg} border border-white shadow-sm p-4 flex items-center gap-3`}>
              <IconComp className={`w-5 h-5 flex-shrink-0 ${color}`} />
              <div>
                <p className={`${isText ? 'text-base' : 'text-2xl'} font-bold ${color}`}>{value}</p>
