...
function CheckBadge({ label, status }) {
  ...
  return (
    <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${bg} min-w-[130px]`}>
      ...
      <div>
        <p className="text-[9px] font-semibold text-slate-500 uppercase tracking-wide leading-none">{label}</p>
    ...
    <div className="space-y-2">
      {docs.map(doc => (
        <div key={doc.id} className="flex items-center gap-3 border border-slate-200 rounded-xl px-4 py-3 hover:bg-slate-50">
          ...
          <span className={`text-[10px] font-semibold ${statusColor[doc.review_status] || 'text-slate-400'}`}>
    ...
    <AdminLayout>
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-slate-900">Doctor Verification Queue</h1>
          ...
          <div className="fixed inset-0 bg-black/80 z-[60] flex items-center justify-center p-6" onClick={() => setSelectedDoc(null)}>
            <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full p-4" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-3">
                <p className="font-semibold text-slate-800 text-sm">{selectedDoc.document_type_label || selectedDoc.file_name}</p>
                <button onClick={() => setSelectedDoc(null)} className="text-slate-400 hover:text-slate-600 font-semibold text-xl leading-none">×</button>
              </div>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-6">
              ...
              <div className="flex items-start justify-between px-6 py-5 border-b border-slate-200">
                <div>
                  <h2 className="text-lg font-semibold text-slate-900">Review: {selected.doctor_name}</h2>
                  <p className="text-sm text-slate-500 mt-0.5">
                    {selected.country} · {selected.specialty || 'Specialty not specified'} · Reg# {selected.registration_number}
                  </p>
                </div>
                <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-600 text-xl font-semibold leading-none mt-1">×</button>
              </div>
              ...
              <div className="px-6 pb-5 space-y-3">
                ...
                {allChecksPassed ? (
                  <Button className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 text-sm"
