          ...
          {travelDates && (
            <div className="grid grid-cols-3 gap-3 bg-green-50 rounded-lg p-4 border border-green-100">
              <div>
                ...
                <p className="text-sm font-bold text-slate-800">{travelDates.arrival}</p>
              </div>
              <div className="border-l border-green-200 pl-3">
                <p className="text-xs text-green-700 font-semibold uppercase">Duration</p>
                <p className="text-sm font-bold text-slate-800">{travelDates.days} days / {travelDates.nights} nights</p>
              </div>
              <div className="border-l border-green-200 pl-3">
                <p className="text-xs text-green-700 font-semibold uppercase">Return</p>
                <p className="text-sm font-bold text-slate-800">{travelDates.return}</p>
              </div>
            </div>
          )}
    ...
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          {/* Origin Legs */}
          <div>
            <p className="text-xs font-bold text-blue-700 uppercase tracking-wide mb-2">🏠 Origin Driver Legs</p>
            <div className="space-y-2">
              {legs.filter(l => l.type === 'origin').map(leg => (
                <div key={leg.label} className="flex justify-between items-center bg-blue-50 rounded-lg px-4 py-3">
                  ...
                  <p className="text-sm font-bold text-blue-800">{leg.value > 0 ? USD(leg.value) : <span className="text-slate-400 font-normal">Pending</span>}</p>
                </div>
              ))}
              {originTotal > 0 && (
                <div className="flex justify-between items-center bg-blue-100 rounded-lg px-4 py-2">
                  ...
                  <p className="text-sm font-bold text-blue-900">{USD(originTotal)}</p>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Destination Legs */}
          <div>
            <p className="text-xs font-bold text-green-700 uppercase tracking-wide mb-2">✈️ Destination Driver Legs</p>
            <div className="space-y-2">
              {legs.filter(l => l.type === 'destination').map(leg => (
                <div key={leg.label} className="flex justify-between items-center bg-green-50 rounded-lg px-4 py-3">
                  <p className="text-sm text-slate-700">{leg.label}</p>
                  <p className="text-sm font-bold text-green-800">{leg.value > 0 ? USD(leg.value) : <span className="text-slate-400 font-normal">Pending</span>}</p>
                </div>
              ))}
              {destTotal > 0 && (
                <div className="flex justify-between items-center bg-green-100 rounded-lg px-4 py-2">
                  ...
                  <p className="text-sm font-bold text-green-900">{USD(destTotal)}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <>
            ...
            <div className="bg-white rounded-xl border border-slate-200 px-5 py-4">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                  <h2 className="font-bold text-slate-900">{caseData.client_name}</h2>
