...
import { Plane, Calendar, CreditCard, CheckCircle2, Users, Loader2, MapPin } from 'lucide-react';
...
export default function BookingsModule() {
  ...
  const [notifyResult, setNotifyResult] = useState(null); // { companions_notified, destination }

  // Load active case so we can show destination and pre-fill the toggle state
  const { data: activeCase } = useQuery({
    queryKey: ['bookings-active-case', user?.email],
    queryFn: async () => {
      const cases = await base44.entities.CaseRecord.filter(
        { client_email: user.email }, '-created_date', 1
      );
      const c = cases[0] || null;
      if (c?.companion_package_requested) setCompanion(true);
      return c;
    },
    enabled: !!user?.email,
    staleTime: 60_000,
  ...
  const { mutate: toggleCompanion, isPending } = useMutation({
    mutationFn: async (on) => {
      const res = await base44.functions.invoke('requestCompanionPackage', { action: on ? 'request' : 'cancel' });
      return { on, data: res.data };
    },
    onSuccess: ({ on, data }) => {
      setCompanion(on);
      if (on && data?.companions_notified !== undefined) {
        setNotifyResult({ companions_notified: data.companions_notified, destination: data.destination });
      } else {
        setNotifyResult(null);
      }
      queryClient.invalidateQueries({ queryKey: ['bookings-active-case', user?.email] });
    },
    onError: () => {
      // Toggle didn't commit — revert the optimistic state
      setCompanion(c => !c);
    },
  });
  ...
  function handleToggle() {
    ...
    setCompanion(next); // optimistic
    toggleCompanion(next);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-6">
        ...
        {/* Destination badge if case loaded */}
        {activeCase?.procedure_country && (
          <div className="flex items-center gap-1.5 mb-4 text-xs text-slate-500">
            <MapPin className="w-3.5 h-3.5 text-pink-400" />
            <span>Your destination: <strong className="text-slate-700">{activeCase.procedure_country}</strong></span>
          </div>
        )}
        ...
        <div className="flex items-center justify-between bg-slate-50 rounded-xl px-4 py-3">
          <div>
            ...
            <p className="text-xs text-slate-500">
              +$650 — includes all logistics
              {companion && <span className="ml-2 text-emerald-600 font-semibold">• Added to invoice</span>}
            </p>
          </div>
          <button
            ...
            aria-label="Toggle companion package"
            className={`w-12 h-6 rounded-full transition-all relative flex-shrink-0 ${companion ? 'bg-emerald-600' : 'bg-slate-300'} disabled:opacity-60`}
          >
            {isPending ? (
              <Loader2 className="absolute inset-0 m-auto w-3.5 h-3.5 text-white animate-spin" />
            ) : (
              <div className={`w-5 h-5 bg-white rounded-full shadow transition-transform absolute top-0.5 ${companion ? 'translate-x-6' : 'translate-x-0.5'}`} />
            )}
          </button>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center justify-between bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-xs font-semibold text-emerald-800">Companion Package — Added to Invoice</p>
              </div>
              <p className="text-sm font-bold text-emerald-700">+$650</p>
            </div>

            {/* Companion search result */}
            {notifyResult && (
              <div className="bg-pink-50 border border-pink-100 rounded-xl px-4 py-3">
                {notifyResult.companions_notified > 0 ? (
                  <p className="text-xs text-pink-800 font-semibold">
                    ✓ {notifyResult.companions_notified} companion{notifyResult.companions_notified !== 1 ? 's' : ''} in <strong>{notifyResult.destination || 'your destination'}</strong> have been notified — they'll accept or decline within 24 hours.
                  </p>
                ) : (
                  <p className="text-xs text-pink-800">
                    No companions are currently registered in your destination. Your coordinator has been alerted and will assign one manually.
                  </p>
                )}
              </div>
            )}
            ...
            {/* Pending (loading) */}
            {isPending && (
              <p className="text-xs text-slate-400 text-center">Searching for companions in your destination…</p>
            )}
          </motion.div>
        )}
      </div>
