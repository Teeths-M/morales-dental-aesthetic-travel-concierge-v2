...
// Generalized from src/components/booking/GuestAuthGate.jsx for the partner
// signup wizards (doctor, taxi service, travel agency). Those wizards already
// auto-save the in-progress form to localStorage (src/lib/signupDraft.js) on
// every change, so unlike the booking gate this one doesn't need to serialize
// the form itself — it only needs to send the person to sign in and bring
// them straight back to the same signup page, where the draft (including
// which step they were on) restores automatically.
export default function SignupAuthGate({ isOpen, redirectPath, title, message, onCancel }) {
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <p style={{ margin: 0, fontSize: 14, color: 'rgba(255,255,255,0.45)', lineHeight: 1.55 }}>
            {message || 'Sign in to finish creating your account. Your form is saved — you\'ll be right back here.'}
          </p>
        </div>
        ...
        <div style={{ background: 'rgba(16,185,129,0.07)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 14, padding: '12px 16px', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              ...
              <p style={{ margin: '2px 0 0', fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>You won't need to re-enter anything.</p>
            </div>
          </div>
        </div>

        <button
          onClick={() => base44.auth.loginWithProvider('google', redirectPath)}
          style={{
        ...
        <button
          onClick={() => { window.location.href = `/login?redirect=${encodeURIComponent(redirectPath)}`; }}
          style={{
