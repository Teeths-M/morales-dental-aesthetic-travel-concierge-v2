...
function Pill({ item, isSelected, onToggle, accent = CALM.action }) {
  return (
    <motion.button
      type="button"
      ...
      style={{
        ...
        border: isSelected ? `2px solid ${accent}` : `1px solid ${CALM.border}`,
        outline: 'none',
      }}
    >
      {item.icon && (
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.desc && (
          <p className="text-xs mt-0.5 leading-tight"
            style={{ color: isSelected ? CALM.textSoft : CALM.textFaint }}>
            {item.desc}
          </p>
        )}
      </div>
...
export default function MedicalHistoryPills({
  ...
  return (
    <div>
      {title && (
        <p className="text-sm font-semibold mb-1" style={{ color: CALM.text }}>{title}</p>
      )}
      {hint && (
        <p className="text-xs mb-3" style={{ color: CALM.textFaint }}>{hint}</p>
      )}
      {!title && !hint && (
        <p className="text-xs mb-3" style={{ color: CALM.textFaint }}>
          Tap all that apply — or tap <strong style={{ color: CALM.textSoft }}>None</strong> if you have no conditions.
        </p>
      )}
      ...
      {selected.length > 0 && !exclusive.includes(selected[0]) && (
        <motion.div
          ...
          style={{ background: CALM.actionSoft, border: `1px solid rgba(14,138,125,0.2)` }}
        >
          <Check className="w-3.5 h-3.5 flex-shrink-0" style={{ color: accent }} />
          <p className="text-xs" style={{ color: CALM.action }}>
