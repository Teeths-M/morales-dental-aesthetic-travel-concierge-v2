...
- **Partner never responds to being offered a job** (doctor, driver, travel agency, companion) — `checkPartnerSLABreaches`: 24h → auto-dispatches a backup, 48h → human-intervention alert. For doctors specifically, this used to be a silent swap — the original doctor got no warning before being replaced. Added 2026-08-05: at the 12h halfway point, a link-only reminder email goes to the doctor (`doctor_email` + their existing `doctor_portal_token` portal link, gated by a new one-time `doctor_reminder_sent_at` field) so the 24h auto-backup is a deadline they were actually told about, not a surprise.
