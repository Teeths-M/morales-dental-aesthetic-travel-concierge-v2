...
- **Patient goes silent / phone dies** — `scheduleSoloCheckIns` creates a check-in for every actively-traveling case (first at 6h post-intake-handshake, then every 12h; covers solo **and** companioned travelers — a companion nearby doesn't guarantee M can reach the patient). `escalateSoloCheckIn` runs the ladder: 2h nudge → 3h guardian alert → 5h `dispatchSoloSecurity` (real SMS handshake with a security agency, tries the next agency if unconfirmed in 15 min, loud failure if every agency is exhausted) → 9h police/embassy escalation. `checkStaleLiveLocations` is a parallel, consent-gated GPS-staleness check.
