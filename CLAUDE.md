...
Every scheduled job below runs from `.github/workflows/safety-cron.yml` (15-min tick, hourly, 6-hourly, and a daily 09:00 UTC deployment audit that fails loudly on anything undeployed) — **not** Base44's own scheduler, and **not** an admin clicking a button. This file's own header documents a recurring bug class worth knowing before adding a new scheduled function: real, correct code that nobody ever actually scheduled, or that required a live admin session (which a cron tick can never hold). This has bitten `checkPartnerSLABreaches`, `dispatchSoloSecurity`, and `scheduleSoloCheckIns` — each existed, worked, and protected nobody until fixed. Any new scheduled safety job must (1) gate on `_shared/cronAuth.ts`'s `cronAuthorized()`, never `base44.auth.me()` + a role check, and (2) actually appear in `safety-cron.yml`, not just in its own header comment describing an intended cadence — trust the workflow file, not a function's self-description of when it runs.

- **Partner never responds to being offered a job** (doctor, driver, travel agency, companion) — `checkPartnerSLABreaches`: 24h → auto-dispatches a backup, 48h → human-intervention alert.
