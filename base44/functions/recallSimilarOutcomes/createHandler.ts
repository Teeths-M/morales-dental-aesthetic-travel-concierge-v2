// DUPLICATED from base44/functions/_shared/createHandler.ts (plus its own transitive
// deps: incidentReporting.ts, rateLimit.ts, validate.ts, emailTemplate.ts — all copied
// alongside this file in this same directory). This function still runs on Base44's
// legacy per-function Deno runtime, which — unlike the Cloudflare runtime the other
// 250+ functions run on — rejects a relative import that reaches outside its own
// directory (Base44 support / Estee, 2026-07-26). Found affecting this function
// 2026-07-28 — it wasn't part of the original 5-function audit (ea036794) since it
// was built after that audit ran; same fix, same root cause. Keep this in sync with
// the canonical _shared/createHandler.ts by hand. Per Base44 support, a clean deploy
// of this self-contained version should migrate this function onto the Cloudflare
// runtime, at which point this duplicate can be deleted and the import in entry.ts
// reverted to '../_shared/createHandler.ts'.
