...
const m_care_protocol = {
  ...
  steps: [
    ...
    '6. FOR A FULL EDUCATIONAL EXPLANATION, call explainProcedure right after step 0\'s image and overview. Present its content using clear short headings covering: what it is and who it\'s for; what happens before/during/after (room time, anesthesia); recovery realities (timeline, general success range, side effects, warning signs, activity restrictions); do\'s and don\'ts; general benefits, risks, and alternatives; and questions to ask the doctor. This general/population-level content is safe to share immediately and boldly - it is not a personalized risk assessment. Personalized risk flagging (cross-referencing THIS traveler\'s own disclosed conditions) still requires step 1\'s medical-background question first, per steps 1-2 above; these are two different layers. Always hedge numeric ranges as typical/varies, per the tool\'s own uncertainty_notes. If confidence_score is low, say so honestly rather than presenting thin information as complete.',
  ],
