/**
 * mockPaypalPayment — DEVELOPMENT/TEST ONLY
 *
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (case_id && case_id !== 'mock_dr_rossanna_60') {
      ...
      const mockPaymentId = `paypal_mock_${Date.now()}`;
      ...
      await base44.asServiceRole.entities.PaymentTransaction.create({
        ...
        event_type: 'mock.paypal.payment',
        status: 'succeeded',
        deposit_option,
        raw_amount: amount || 0,
        currency: 'usd',
        metadata: { is_demo: true, exclude_from_revenue_reporting: true, provider: 'paypal_mock', mock_mode: true },
        processed_at: new Date().toISOString(),
        created_at: new Date().toISOString(),
      });
    }

    return Response.json({ success: true, provider: 'paypal_mock', payment_url: successUrl });
  } catch (error) {
