...
function sanitizePromptInput(input: unknown, maxLen = 1500): { text: string; flagged: boolean; hits: string[] } {
  ...
  for (const re of INJECTION_PATTERNS) {
    if (re.test(text)) { hits.push('injection'); text = text.replace(re, ' [removed] '); }
    re.lastIndex = 0;
  }
...
Deno.serve(async (req) => {
  try {
    ...
    if (!partner_id || !registered_name) return Response.json({ error: 'partner_id and registered_name are required' }, { status: 400 });

    const isAdminCaller = user?.role === 'admin' || user?.role === 'platform_admin';
    if (!isAdminCaller) {
      if (partner_type !== 'doctor') return Response.json({ error: 'Unauthorized' }, { status: 403 });
      const own = await base44.asServiceRole.entities.Doctor.get(partner_id).catch(() => null as any);
      if (!own || own.email !== user?.email) return Response.json({ error: 'Unauthorized — doctors may only trigger their own scan' }, { status: 403 });
    }
    ...
    // ── Phase 4: Fraud graph cross-reference ─────────────
    if (phone) {
    ...
    // ── Phase 5: Domain + social (skip if already critical) ──────
...
Return JSON only:
{
  ...
  "google": { "status": "verified"|"unverified"|"not_found", "rating": null|number, "review_count": null|number, "snippet": null|string }
    ...
    if (partner_type === 'doctor')
      await base44.asServiceRole.entities.Doctor.update(partner_id, { ...intelPatch, ...statusPatch }).catch(() => {});
    else if (partner_type === 'travel_agency')
      await base44.asServiceRole.entities.TravelAgency.update(partner_id, statusPatch).catch(() => {});
    else if (partner_type === 'taxi')
      await base44.asServiceRole.entities.TaxiService.update(partner_id, statusPatch).catch(() => {});
    else if (partner_type === 'security')
      await base44.asServiceRole.entities.SecurityAgency.update(partner_id, statusPatch).catch(() => {});
...
<p>This application has been held for manual review. Review in the Partner Command Center at /admin/command-center</p>`,
        }).catch(() => {});
      }
    }
